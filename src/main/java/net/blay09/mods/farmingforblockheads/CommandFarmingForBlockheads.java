...
public class CommandFarmingForBlockheads {
    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        dispatcher.register(Commands.literal("farmingforblockheads").requires(it -> it.hasPermissionLevel(2)).then(Commands.literal("reload").executes(it -> {
            ...
            it.getSource().sendFeedback(new TranslationTextComponent("commands.farmingforblockheads:reload.success"), true);
            if (AbstractRegistry.registryErrors.size() > 0) {
                it.getSource().sendFeedback(new StringTextComponent(TextFormatting.RED + "There were errors loading the Farming for Blockheads registries:"), true);
                TextFormatting lastFormatting = TextFormatting.WHITE;
                for (String error : AbstractRegistry.registryErrors) {
                    it.getSource().sendFeedback(new StringTextComponent(lastFormatting + "* " + error), true);
