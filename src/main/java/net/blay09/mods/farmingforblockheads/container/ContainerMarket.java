...
public class ContainerMarket extends Container {
    ...
    private final InventoryBasic marketInputBuffer = new InventoryBasic(new TextComponentTranslation("container.farmingforblockheads:market"), 1);
    private final InventoryBasic marketOutputBuffer = new InventoryBasic(new TextComponentTranslation("container.farmingforblockheads:market"), 1);
    ...
    public ContainerMarket(EntityPlayer player, BlockPos pos) {
        ...
        addSlot(new SlotMarketBuy(this, marketOutputBuffer, 0, 61, 39));
    ...
    public void detectAndSendChanges() {
        ...
        if (!player.world.isRemote && !sentItemList) {
            NetworkHandler.channel.send(PacketDistributor.PLAYER.with(() -> (EntityPlayerMP) player), new MessageMarketList(MarketRegistry.getGroupedEntries()));
            sentItemList = true;
        }
    }
